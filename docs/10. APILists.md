...
**Canonical contract:** [FInal/03. ApiSpecification.yaml](11.%20ArchitectureDesign/FInal/03.%20ApiSpecification.yaml). If a path in this catalog disagrees with that file, **OpenAPI wins**. Extra endpoints below are backlog/future unless added to OpenAPI.
...
**Internal service-to-service APIs (not on the public Gateway)**
```
POST /internal/v1/ledger/entries
GET  /internal/v1/ledger/accounts/{accountId}
GET  /internal/v1/ledger/accounts/{accountId}/entries
...
Copied from Final OpenAPI (prefix `/api/v1` required).
