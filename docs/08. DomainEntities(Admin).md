...
OpenAPI documents the HTTP security role **`ADMIN`** for `/api/v1/admin/**`. Fine-grained staff types below are **rows in `admin_db.roles`**, assigned via `USER_ROLES` — not extra Gateway JWT role names unless product later promotes them.

Seed at least `ADMIN`. Optionally seed:
